class Statistic::RakeTaskWrapper
  ...
  def initialize(klass, _namespaces=nil)
    @klass = klass
    @namespaces = _namespaces || [] #[klass.name.underscore.gsub(/^.*\//,'').to_sym]
  end

  def namespace(name)
    yield self.class.new(@klass, @namespaces + [name])
  end

  def method_missing(task_name, description=nil)
    code_block = lambda{yield}
    specification = Statistic::RakeTaskSpecification.new(task_name, :description => description, :block => code_block, :namespaces => @namespaces, :klass_name => @klass.name)
